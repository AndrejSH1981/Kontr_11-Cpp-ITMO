...
public:
    ...
    explicit Time(int hours, int minutes, int seconds) : Time(hours, minutes, seconds, true) {
    ...
    friend Time operator+(Time& lhs, Time& rhs) {
        (lhs.To_Time_Other() + rhs.To_Time_Other());
        return Result(lhs.To_Time_Other() + rhs.To_Time_Other());
    }
    friend Time operator+(Time& lhs, double rhs) {
        (lhs.To_Time_Other() + 111111.0);
        return Result(lhs.To_Time_Other() + 111111.0);
    }
    friend Time operator-(Time& lhs, Time& rhs) {
        (lhs.To_Time_Other() - rhs.To_Time_Other());
        return Result(lhs.To_Time_Other() - rhs.To_Time_Other());
    ...
    friend bool operator==(Time& lhs, Time& rhs) {
        return lhs.To_Time_Other() == rhs.To_Time_Other();
    }
    friend bool operator>(Time& lhs, Time& rhs) {
        return lhs.To_Time_Other() > rhs.To_Time_Other();
    }
    ...
    string comparison(Time& other) {
        string comp;
        if (To_Time_Other() > other.To_Time_Other()) {
            comp = "\n - первое время больше второго - ";
        }
        if (To_Time_Other() == other.To_Time_Other()) {
            comp = "\n - первое и второе время равны - ";
        }
        if (To_Time_Other() < other.To_Time_Other()) {
            comp = "\n - второе время больше первого - ";
        }
...
int main() {
    ...
    string time_str;
    time_str = t1_time.comparison(t2_time);
    cout << time_str << endl;
    cout << "--------------" << endl;
    ...
    cout << "\nt1 + t2: " << t1_time + t2_time << " (operatot+) " << endl;
    cout << "--------------" << endl;
    cout << "\nt1 - t2: " << t1_time - t2_time << " (operatot-) " << endl;
    cout << "--------------" << endl;
    double q = 0;
    cout << "\nt1 + double(111111.0) = " << t1_time + q << " (operatot+) " << endl;
    cout << "--------------" << endl;
    cout << "\nt1 - double(111111.0) = " << t1_time - q << " (operatot-) " << endl;
    cout << "--------------" << endl;
    string str;
